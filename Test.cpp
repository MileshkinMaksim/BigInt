...
int main() {
  ...
  std::cout << "Using long long to check: " << (LLSUP ? "True" : "False")
            << endl;
  std::cout << "Number of operation calls: " << CNT_OF_REPEATS_BENCHMARKING
            << endl;
  std::cout << "Number of operation calls in block: "
            << CNT_OF_ONE_BLOCK_BENCHMARKING << endl;
  ...
  if (exsistOper('&') || exsistOper('|') || exsistOper('^')) {
    std::cout
        << "[Warning] Binary operations are checked only on positive numbers"
        << endl;
    haveBinary = true;
  }
  std::cout << "Checking using positive numbers: "
            << (exsistSign(1) ? "True" : "False") << endl;
  std::cout << "Checking using negative numbers: "
            << (exsistSign(-1) ? "True" : "False") << endl;
  std::cout << endl << endl;
  ...
  do {
    ...
    for (size_t block_num = 0;
         block_num <= CNT_OF_ONE_BLOCK_BENCHMARKING && is_correct;
         block_num++) {
      ...
      try {
        ...
        std::cout << "\tAn error occurred in the test. Continue testing"
                  << endl;
      }
      test_num++;
      if (is_correct == false) {
  ...
  std::cout << total_time << "ms total work time ("
            << CNT_OF_REPEATS_BENCHMARKING << " cycles)" << endl;
  std::cout << total_time / timers.size() << "ms ("
            << CNT_OF_ONE_BLOCK_BENCHMARKING << " cycles)" << endl;
  std::cout << (double)total_time / timers.size() /
                   CNT_OF_ONE_BLOCK_BENCHMARKING
            << "ms on 1 cycle" << endl;
...
bool test_twoL(BigInt& a, char& oper, BigInt& b) {
  ...
  LOG(std::cout << " = [" << my_ans << "] / {" << correct_ans << "} ";
      std::cout << (is_correct ? "CORRECT" : "!!WRONG!!"); std::cout << endl;)
  return is_correct;
}
...
int generateBIGint() { return ((int)rand() + ((int)rand() << 16)); }
int generateSMALLint() { return ((int)rand()); }
